...
function _chart(d3,width,data,forceCollide,drag,invalidation)
{
  ...
  //const width = 300;
  const height = 900;  // KJS
  const margin = 00; // to avoid clipping the root circle stroke
  ...
  // 수정후
  const maxValue = d3.max(data[0], d => d.value);  // 또는 data.flat() 등

  var sizeScale = d3.scaleSqrt()
       .domain([0, maxValue])
       .range(maxValue <= 100 ? [10, 50] : [10, 100]);  // 최소 5 보장
  ...
  // Add a title.   JSK 화면 출력
  node.append("title")
      .text(d => `${d.data.id}\n  ${d.data.purl} , GAID : ${format(d.data.value)},  PV : ${format(d.data.p)},  MID : ${format(d.data.m)}`);

  // Add a filled circle.
  const circle = node.append("circle")
      .attr("fill-opacity", 0.9)
      .attr("fill", d => color(group(d.data)))
      .attr("r", d => d.r * 0.2);  // 반지름
      // .attr("cx", d => d.x)
      // .attr("cy", d => d.y)
      // .call(drag(simulation));
       //.attr("r", d => d.r);
...
//console.log(nodes);
  ...
  circle.transition()
      ...
      .duration(750)
      .attrTween("r", d => {
        //const i = d3.interpolate(0, d.r * 0.5); // 반지름 원본
        const i = d3.interpolate(0, d.r * 1); // 반지름
        return t => d.r = i(t);
      });
...
.${id}-swatch {
  ...
  rvalue =  htl.html`<div style="display: flex; align-items: center; min-height: 33px; margin-left: ${+marginLeft}px; font: 20px sans-serif;">
                  <style>
                .${id} {
                  display: inline-flex;
                  align-items: center;
