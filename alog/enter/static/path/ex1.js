...
function _chart(d3,data)
{
  ...
  const svg = d3.create("svg")
      ...
      .attr("style", "max-width: 100%; height: 800px; font: 12px sans-serif;")
...
.${id}-swatch {
  ...
  <div style=${{width: "100%", columns}}>${domain.map(value => {
    ...
    return htl.html`<div class=${id}-item>
      <div class=${id}-swatch style=${{background: color(value)}}>AREA 4</div>
...
.${id}::before {
  ...
  <div>AREA 1 ${domain.map(value => htl.html`<span class="${id}" style="--color: ${color(value)}">${format(value)}</span>`)}</div>`;
}
)}
