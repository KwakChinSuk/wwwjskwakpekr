...
function _chart(d3,width,data,forceCollide,drag,invalidation)
{
  ...
  // KJS  그룹 개수
  const root = pack(d3.hierarchy({children: [{children: data[0]}, {children: data[1]}, {children: data[2]}, {children: data[3]}, {children: data[4]} ]})
    .sum(d => d.value));
...
function _forceCollide(d3){
    return(
        function forceCollide() {
          ...
          const padding1 = 10; // KJS  간격 separation between same-color nodes
...
function Swatches(color, {
  ...
  if (columns !== null) return htl.html`112<div style="display: flex; align-items: center; margin-left: ${+marginLeft}px; min-height: 33px; font: 10px sans-serif;">333
  <style>
...
.${id}-swatch {
  ...
  <div style=${{width: "100%", columns}}>${domain.map(value => {
    ...
    return htl.html`111<div class=${id}-item>
  ...
  return htl.html`<div style="display: flex; align-items: center; min-height: 33px; margin-left: ${+marginLeft}px; font: 20px sans-serif;">TOP 50 &nbsp;&nbsp;&nbsp;
                  <style>
                .${id} {
                  display: inline-flex;
                  align-items: center;
