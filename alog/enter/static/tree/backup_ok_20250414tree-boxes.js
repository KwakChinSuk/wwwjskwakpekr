...
function treeBoxes(urlService, jsonData)
{
	...
	var rectNode = { width : 220, height : 50, textMargin : 2 },
...
width  =2000
		...
		var maxTreeWidth = breadthFirstTraversal(tree.nodes(root), function(currentLevel) {
			...
			currentLevel.forEach(function(node) {
				if (node.t == 't1')	node.color = t1;
				if (node.t == 't2')	node.color = t2;
				if (node.t == 't3')	node.color = t3;
				if (node.t == 't4')	node.color = t4;
				if (node.t == 't5')	node.color = t5;
				if (node.t == 't6')	node.color = t6;
				if (node.t == 't7')	node.color = t7;
				if (node.t == 't8')	node.color = t8;
				if (node.t == 't9')	node.color = t9;
				if (node.t == 't0')	node.color = t0;
				});
			});
		maxTreeWidth = 10;
		maxDepth = 15
